...
runs in a specific workflow, while ~pychum~ is the *sole* path for *single-run* input generation. Parsing/plotting: ~chemparseplot~ (OPI only as an internal backend). ~wailord~: optional batch/experiment shell via those packages.
